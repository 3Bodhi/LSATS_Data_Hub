...
-- ============================================================================
-- SILVER LAYER: TDX-Specific Departments (Source-Specific Table)
-- ============================================================================

-- Source-specific table for TeamDynamix department data with full fidelity
-- This table stores ALL TDX department data including custom attributes
-- Separate from the consolidated silver.departments which merges TDX + UMICH API
...
-- Create trigger for automatic timestamp updates
CREATE TRIGGER update_silver_tdx_departments_updated_at
    BEFORE UPDATE ON silver.tdx_departments
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Indexes for common queries
CREATE INDEX idx_tdx_departments_dept_code ON silver.tdx_departments(dept_code);
CREATE INDEX idx_tdx_departments_is_active ON silver.tdx_departments(is_active);
CREATE INDEX idx_tdx_departments_is_enriched ON silver.tdx_departments(is_enriched);
CREATE INDEX idx_tdx_departments_updated_at ON silver.tdx_departments(updated_at);
...
CREATE INDEX idx_tdx_departments_quality_score ON silver.tdx_departments(data_quality_score DESC);
CREATE INDEX idx_tdx_departments_entity_hash ON silver.tdx_departments(entity_hash);
