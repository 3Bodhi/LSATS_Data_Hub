...
COMMENT ON COLUMN silver.ad_organizational_units.distinguished_name IS 'Full distinguished name (DN). Unique but can change if OU is renamed/moved. Example: OU=psyc-danweiss,OU=Psychology,OU=EHTS,... Unbounded text (AD DNs have no fixed max length). 100% populated.';
