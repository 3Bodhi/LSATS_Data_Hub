...
class TeamDynamixFacade:
    ...
    def get_last_requestor_response(
        self, ticket_id: str, requestor_name: str = None
    ) -> datetime.datetime:
        """
        ...
        if requestor_entries:
            latest_response = max(
                requestor_entries, key=lambda x: x.get("CreatedDate", "")
            )
    ...
    def days_since_requestor_response(
        self, ticket_id: str, requestor_name: str = None
    ) -> int:
        """
    ...
    def get_contextual_summary(self, ticket_id: str, recent_count: int = 15) -> dict:
        ...
        full_conversation = self.get_conversation(
            ticket_id=ticket_id, exclude_system=True
        )
        ...
        summary = (
            f"Earlier conversation ({len(older_messages)} messages from "
    ...
    def _is_system_message(self, entry: dict) -> bool:
        ...
        return sender == "system" or any(
            indicator in message for indicator in system_indicators
        )
    ...
    def _merge_consecutive_messages(self, conversation: list) -> list:
        ...
        for next_msg in conversation[1:]:
            if next_msg["sender"] == current_msg[
                "sender"
            ] and self._messages_within_timeframe(
                current_msg["timestamp"], next_msg["timestamp"], minutes=30
            ):
                # Merge messages
    ...
    def _messages_within_timeframe(
        self, timestamp1: str, timestamp2: str, minutes: int = 30
    ) -> bool:
