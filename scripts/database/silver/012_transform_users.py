...
class UserConsolidationService:
    ...
    def _fetch_source_records(
        ...
        try:
            ...
            """
            tdx_records = self.db_adapter.query_to_dataframe(tdx_query, params).to_dict("records")
            ...
            """
            ad_records = self.db_adapter.query_to_dataframe(ad_query, params).to_dict("records")
            ...
            WHERE uniqname IS NOT NULL {time_filter}
            """
            umapi_records = self.db_adapter.query_to_dataframe(umapi_query, params).to_dict("records")
            ...
            {time_filter}
            """
            mcom_records = self.db_adapter.query_to_dataframe(mcom_query, params).to_dict("records")
            ...
            def get_group(u):
                ...
                if u not in grouped_data:
                    grouped_data[u] = {'tdx': None, 'ad': None, 'umapi': [], 'mcom': None}
            ...
            logger.info(f"📦 Consolidated {len(grouped_data)} unique users from sources")
            return grouped_data
    ...
    def _aggregate_umapi_records(self, umapi_records: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        aggregated = {
            ...
            'job_title': primary.get('university_job_title') or primary.get('department_job_title'),
            ...
            'department_ids': [
                ...
                for rec in sorted_records if rec.get('department_id')
            ],
            'job_codes': [
                {
                ...
                for rec in sorted_records if rec.get('jobcode')
            ],
            'supervisor_ids': [
                {
                    'empl_id': rec.get('supervisor_id'),
                    'empl_rcd': rec.get('empl_rcd')
                }
                for rec in sorted_records if rec.get('supervisor_id')
            ],
            'umich_empl_ids': list({rec.get('empl_id') for rec in sorted_records if rec.get('empl_id')})
        }
    ...
    def _merge_user_records(
        ...
        """
        sources = []
        if tdx_record: sources.append('tdx')
        if ad_record: sources.append('ad')
        if umapi_records: sources.append('umapi')
        if mcom_record: sources.append('mcom')
        ...
        last_name = pick_first(
            ...
            mcom_record.get('sn') if mcom_record else None, # MCom might use sn? schema says display_name/given_name. Let's assume standard LDAP attrs or what's in schema.
        ...
        # --- Status ---
        is_active = False
        if tdx_record and tdx_record.get('is_active'): is_active = True
        if ad_record and not ad_record.get('ad_account_disabled'): is_active = True
        if umapi_records: is_active = True # Employees are active
        if mcom_record: is_active = True # MCom users usually active if present? Or we assume active.
        
        is_employee = bool(umapi_records) or (tdx_record and tdx_record.get('is_employee'))
        ...
        merged = {
            ...
            "ad_object_guid": ad_record.get('ad_user_guid') if ad_record else None, # ad_user_guid is the PK in silver.ad_users
            "ad_sam_account_name": ad_record.get('sam_account_name') if ad_record else None,
            ...
            "ad_ou_organization": ad_record.get('ou_organization') if ad_record else None,
            ...
            # POSIX
            "home_directory": mcom_record.get('home_directory') if mcom_record else None,
            ...
            "tdx_security_role_name": tdx_record.get('security_role_name') if tdx_record else None,
    ...
    def _calculate_content_hash(self, merged_record: Dict[str, Any]) -> str:
        ...
        exclude_fields = {'data_quality_score', 'quality_flags', 'entity_hash', 'ingestion_run_id', 'created_at', 'updated_at'}
        
        hash_payload = {k: v for k, v in merged_record.items() if k not in exclude_fields}
    ...
    def _calculate_data_quality(self, merged_record: Dict[str, Any]) -> Tuple[Decimal, List[str]]:
        ...
        if 'tdx' in sources and 'umapi' in sources and 'ad' in sources and 'mcom' in sources:
            score += Decimal("0.10")
            
        return max(Decimal("0.00"), min(Decimal("1.00"), score)), flags

    def _batch_upsert_records(self, records: List[Dict[str, Any]], run_id: str, dry_run: bool = False) -> Tuple[int, int, int]:
        """
        ...
        try:
            # Prepare data
            json_fields = [
                'umich_empl_ids', 'department_ids', 'job_codes', 'supervisor_ids',
                'mcommunity_ou_affiliations', 'ad_group_memberships', 'tdx_group_ids',
                'ad_ou_full_path', 'quality_flags'
            ]
            ...
            users_table = table('users', 
                column('uniqname'), column('tdx_user_uid'), column('umich_empl_id'), column('umich_empl_ids'),
                column('ldap_uid_number'), column('ldap_gid_number'), column('ad_object_guid'), column('ad_sam_account_name'),
                column('first_name'), column('last_name'), column('full_name'), column('display_name'),
                column('primary_email'), column('work_phone'), column('mobile_phone'),
                column('department_id'), column('department_name'), column('department_ids'),
                column('job_title'), column('department_job_title'), column('primary_job_code'), column('job_codes'),
                column('primary_supervisor_id'), column('supervisor_ids'), column('reports_to_uid'),
                column('is_pi'), column('is_active'), column('is_employee'),
                column('ad_account_disabled'),
                column('mcommunity_ou_affiliations'),
                column('ad_group_memberships'), column('tdx_group_ids'),
                column('ad_ou_root'), column('ad_ou_organization'), column('ad_ou_department'), column('ad_ou_full_path'),
                column('home_directory'), column('login_shell'),
                column('tdx_external_id'), column('tdx_beid'), column('tdx_security_role_name'),
                column('data_quality_score'), column('quality_flags'),
                column('source_system'), column('source_entity_id'), column('entity_hash'), column('ingestion_run_id'),
                column('updated_at')
            )
            ...
            update_cols = {c.name: c for c in stmt.excluded if c.name not in ['uniqname', 'created_at']}
            ...
            with self.db_adapter.engine.connect() as conn:
                with conn.begin():
                    ...
                    return processed, 0, skipped # We can't easily distinguish created vs updated in bulk
    ...
    def consolidate_users(self, full_sync: bool = False, dry_run: bool = False, exclude_alumni: bool = False):
        ...
        try:
            ...
            # 2. Fetch Sources
            source_data = self._fetch_source_records(last_run, full_sync, exclude_alumni)
            
            if not source_data:
                logger.info("✨ No records to process")
                if not dry_run: self.complete_transformation_run(run_id, stats)
                return
            ...
            # 4. Finish
            duration = (datetime.now(timezone.utc) - datetime.fromisoformat(stats["started_at"])).total_seconds()
...
def main():
    ...
    parser.add_argument("--exclude-alumni", action="store_true", help="Exclude alumni-only users")
