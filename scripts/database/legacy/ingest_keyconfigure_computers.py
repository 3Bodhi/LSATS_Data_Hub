...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("logs/keyconfigure_computer_ingestion.log"),
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    if existing_hash is None:
                        ...
                        logger.debug(
                            f"Computer unchanged, skipping: {name} (MAC: {mac})"
                        )
                        should_insert = False
...
def main():
    ...
    try:
        ...
        # Create and run KeyConfigure ingestion service
        ...
        print("💻 Starting KeyConfigure computer ingestion with content hashing...")
        results = (
        ...
        print(f"     └─ Operating Systems: {len(results['os_types'])}")
        ...
        if "os_summary" in computer_analyses:
            ...
            for _, row in os_summary.head(20).iterrows():
                print(f"   - {row['os']}: {row['computer_count']} computers")

        # CPU distribution
        if "cpu_summary" in computer_analyses:
            print("\n🔧 Top 10 CPU Types:")
            cpu_summary = computer_analyses["cpu_summary"]
            for _, row in cpu_summary.head(10).iterrows():
                print(f"   - {row['cpu']}: {row['computer_count']} computers")
        ...
        if "features_summary" in computer_analyses:
            ...
            print(f"   - Total RAM: {features['total_ram_gb']:.2f} GB")
            print(f"   - Total Disk: {features['total_disk_gb']:.2f} GB")
            print(f"   - Average RAM per Computer: {features['avg_ram_gb']:.2f} GB")
            print(f"   - Average Disk per Computer: {features['avg_disk_gb']:.2f} GB")

        # Clean up
        ingestion_service.close()

        print("\n✅ KeyConfigure computer ingestion completed successfully!")
