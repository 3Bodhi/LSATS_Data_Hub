...
class MCommunityUserIngestionService:
    ...
    def __init__(
        ...
        logger.info(
            f"MCommunity user ingestion service initialized with content hashing "
        ...
        try:
            ...
            for user_data in raw_users:
                try:
                    ...
                    if should_insert:
                        if self.dry_run:
                            logger.info(f"[DRY RUN] Would insert user {uid_number}")
                        else:
...
def main():
    ...
    try:
        ...
        parser = argparse.ArgumentParser(
            description="Ingest MCommunity users into bronze layer"
        )
        parser.add_argument(
            "--full-sync",
            action="store_true",
