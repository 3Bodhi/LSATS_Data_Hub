            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    if existing_hash is None:
                        ...
                        logger.debug(
                            f"⏭️  Award record unchanged, skipping: {composite_id}"
                        )
                        should_insert = False
                    ...
                    if should_insert:
                        if self.dry_run:
                            logger.info(f"[DRY RUN] Would insert award {composite_id}")
                        else:
...
def main():
    ...
    try:
        ...
        parser = argparse.ArgumentParser(
            description="Ingest lab awards from CSV to bronze layer"
        )
        parser.add_argument(
            "--full-sync",
            action="store_true",
            help="Process all records (bypass timestamp filtering)",
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
