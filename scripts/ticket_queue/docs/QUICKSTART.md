...
python -m scripts.ticket_queue.ticket_queue_daemon \
  --report-id 1344 \
  --daemon \
  --interval 300 \
  --log daemon.log
...
```bash
# Help
python -m scripts.ticket_queue.ticket_queue_daemon --help

# Debug mode
python -m scripts.ticket_queue.ticket_queue_daemon --report-id 1344 --log-level DEBUG --dry-run
